...
class ARMPicomBundleFactory:
    ...
    @classmethod
    def create_bundle(cls, options: ARMPicomBundleOptions | None = None) -> ARMPicomBundle:
        '''
            Creates the armpicom bundle with optional pre-configured options.

            :param options: The pre-configured options for the armpicom bundle.
        ...
        info_file = options.get(ARMPicomBundleKeys.OPTION_INFO_FILE) if options else cls._info_file
        ...
        cli_bundle: CLIBundle = CLIBundleRegistry.create_bundle(
            dependencies=CLIBundleDependencies(
                ...
                commands=[gen_picom_bundle]
            )
        )

        cli: CLI = CLI(cli_bundle)

        return ARMPicomBundleRegistry.create_bundle(
            dependencies=ARMPicomBundleDependencies(
                base=base_bundle,
                service=service,
